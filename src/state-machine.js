...
// HELPER FOR FLEXIBLE OPTION MATCHING (NUMBERS & KEYWORDS)
...
function processWhatsAppMessage(senderJid, textInput) {
  ...
  if (state === 'welcome' || state === 'profile_select' || state === 'ask_user_type') {
    const profileChoice = matchInput(text, {
      colaborador: ['1', 'colaborador', 'empleado', 'activo'],
  ...
  if (state === 'data_consent') {
    const consentChoice = matchInput(text, {
      yes: ['1', 'si', 'sí', 'autorizo', 'acepto'],
      no: ['2', 'no', 'rechazo', 'denegar']
    });
  ...
  if (state === 'main_menu' || lower === 'menu') {
    ...
    const menuChoice = matchInput(text, {
      ...
      reporte: ['2', 'reporte', 'reportes', 'novedad', 'accidente', 'ausentismo'],
      institucional: ['3', 'institucional', 'rit', 'copasst', 'cocola', 'brigada', 'boletin', 'boletín'],
    ...
    return formatResponse(`La opción ingresada no es válida. Por favor selecciona una de las opciones disponibles enviando el *número* (1-5) o la *palabra clave*:\n\n` + getMainMenuText());
  }
  ...
  if (state === 'menu_consultas') {
    ...
    if (consultaChoice === 'planilla') {
      sessionManager.updateSession(senderJid, { flowState: 'after_action' });
      return formatResponse(`*Planilla de Seguridad Social*\n\nTu planilla integrada de liquidación de aportes está disponible para descarga en el portal oficial:\n🔗 https://mscorp.com.co/\n\n` + getAfterActionPrompt());
    }
  ...
  if (state === 'sub_comprobante') {
    if (text === '1' || lower.includes('ultimo') || lower.includes('último')) {
      sessionManager.updateSession(senderJid, { flowState: 'after_action' });
  ...
  if (state === 'beneficios_menu') {
    ...
    const choice = matchInput(text, {
      ...
      escucha: ['2', 'escucha', 'psicologa', 'psicóloga', 'daniela', 'monroy'],
      todos: ['3', 'todos', 'ver'],
      volver: ['4', 'volver', 'consultas']
    });
  ...
  if (state === 'menu_reportes') {
    const reportChoice = matchInput(text, {
      ...
      accidente: ['2', 'accidente', 'sst', 'emergencia', 'caida', 'caída'],
      novedad: ['3', 'novedad', 'nomina', 'nómina', 'pago', 'descuento'],
      volver: ['4', 'volver', 'menu']
    });
  ...
  if (state === 'menu_institucional') {
    ...
    if (choice === 'copasst') {
      sessionManager.updateSession(senderJid, { flowState: 'after_action' });
      return formatResponse(`*Comité Paritario SST (COPASST MS Corp):*\n• Laura Martínez (Presidenta)\n• Mauricio Vega (Secretario)\n• Sandra Ríos (Vocal)\n• Felipe Torres (Vocal)\n\n` + getAfterActionPrompt());
    }
    if (choice === 'brigada') {
      sessionManager.updateSession(senderJid, { flowState: 'after_action' });
